...
def search_in_google_es(args: List[str]) -> str:
    ...
    if "knowledge_graph" in response and "description" in response["knowledge_graph"]:
        ...
        "related_questions" in response
        and len(response["related_questions"]) > 0
        and "answer" in response["related_questions"][0]
    ):
        answer = response["related_questions"][0]["answer"]
    elif (
        "organic_results" in response
        and len(response["organic_results"]) > 0
        and "snippet" in response["organic_results"][0]
    ):
